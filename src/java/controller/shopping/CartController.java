...
public class CartController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        User c_user = Utility.getLoginAccount(request, response);

        Cart cart = new Cart(Utility.getStringFromCookie(request, "c_cart"), pList);
        request.setAttribute("cart", cart);
//        if (userdao.isAdmin(c_user.getEmail())) {
//            response.sendRedirect(request.getContextPath() + "/admin/dashboard");
//        } else {
            request.getRequestDispatcher("view/shopping/cart.jsp").forward(request, response);
//        }
    }
