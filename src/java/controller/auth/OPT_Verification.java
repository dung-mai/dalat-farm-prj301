 ...
 * @author Admin
 */
public class OPT_Verification extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        request.getRequestDispatcher("../../view/auth/otp-enter.jsp").forward(request, response);
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String email;
        if (unverifiedEmail == null && changeEmail == null) {
            ...
            if (resend.equals("true")) {
                ...
                try {
                    ...
                    Logger.getLogger(OPT_Verification.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
            String otp = request.getParameter("otp");
            if (!otp.equals(token.getToken_string())) {
                request.setAttribute("mess", "Mã OTP không đúng");
                request.getRequestDispatcher("../../view/auth/otp-enter.jsp").forward(request, response);
            } else {
                if (unverifiedEmail != null) {
                    userDAO.updateVerified(unverifiedEmail);
                    request.getSession().removeAttribute("unverifiedEmail");
                    User newUser = userDAO.getUserByEmail(unverifiedEmail);
                    request.getSession().setAttribute("user", newUser);
                    response.sendRedirect(request.getContextPath() + "/home");
                } else if (changeEmail != null) {
                    request.getSession().setAttribute("acceptchange", true);
                    response.sendRedirect(request.getContextPath() + "/account/new-password");
                }
