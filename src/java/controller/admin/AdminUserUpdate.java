...
public class AdminUserUpdate extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet AdminUserUpdate at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        int id = Utility.parseIntParameter(request.getParameter("id"), -1);
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (avatar == null || avatar.equals("")) {
            userDAO.updateUserInfor(firstname, lastname, phone, address, thisUser.getEmail());
        } else {
            avatar = "images/" + avatar;
            userDAO.updateUserInfor(firstname, lastname, phone, address, avatar, thisUser.getEmail());
        }

//        if (status > 0) {
            userDAO.updateUserStatus(2, id);
//        }

        // UPdate user information to send to JSP
        ...
        if (avatar != null && !avatar.trim().equals("")) {
            thisUser.setAvatar(avatar);
        }
        thisUser.setPhone_number(phone);
        request.setAttribute("user", thisUser);
        request.getRequestDispatcher("../../view/admin/admin-user-detail.jsp").forward(request, response);
    }
